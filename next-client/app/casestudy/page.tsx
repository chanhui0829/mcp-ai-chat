...
import { FiAlertCircle, FiArrowRight, FiArrowLeft, FiTarget, FiShield, FiTrendingUp } from 'react-icons/fi';
    ...
    <div className="flex-1 h-full overflow-y-auto bg-[#f8f9fc] sidebar-scroll">
      <div className="max-w-4xl mx-auto px-6 py-16 space-y-24">
            ...
            <div className="space-y-5">
              ...
              <p className="text-base leading-8 font-medium text-gray-500">
                ...
                **AbortController**를 활용하여 네트워크 레이어에서의 안정성을 확보했습니다.
                또한 Next.js 16 App Router로 마이그레이션하여 SSR 구조를 도입했습니다.
              </p>
            </div>
        ...
        <div className="space-y-10">
          ...
          <Trouble
            title="CSR에서 Next.js SSR로의 아키텍처 마이그레이션"
            problem="기존 React SPA(CSR) 구조에서 초기 로딩 속도가 느리고 SEO 최적화가 어려운 문제가 있었습니다."
            cause="클라이언트 사이드 렌더링으로 인해 첫 페이지 로드 시 자바스크립트 번들 다운로드 후에야 콘텐츠가 표시되어 사용자 경험 저하가 발생했습니다."
            solution="Next.js 16 App Router를 도입하여 SSR(Server-Side Rendering) 구조로 마이그레이션하고, 동적 라우팅 params의 Promise 처리를 적용했습니다."
            decision="초기 로딩 성능과 SEO 최적화를 위해 CSR에서 SSR로의 전면적인 아키텍처 변경을 수행했습니다."
            before="첫 페이지 로드 시 빈 화면 표시 후 자바스크립트 실행으로 콘텐츠 렌더링. LCP 지표 저하."
            after="서버 사이드에서 미리 렌더링된 HTML 전달로 즉각적인 콘텐츠 표시. LCP 개선 및 SEO 최적화 달성."
          />
        </div>
        ...
        <div className="relative group">
          ...
          <div className="relative bg-white border border-blue-50 p-12 rounded-[3rem] space-y-10 shadow-sm">
            ...
            <p className="text-lg text-gray-500 leading-relaxed font-medium italic border-l-4 border-blue-100 pl-6 py-2">
              "단순한 기능 구현을 넘어, CSR에서 SSR로의 아키텍처 마이그레이션과 네트워크 레벨의 흐름 제어가
              사용자 경험에 얼마나 결정적인 역할을 하는지 깊이 체감했습니다. 기술적 원리를 파고들어
              문제를 해결하는 프론트엔드 개발자의 가치를 증명할 수 있었습니다."
            </p>
          </div>
        </div>
      </div>
